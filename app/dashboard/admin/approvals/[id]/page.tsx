...
export default function AdminPropertyReviewPage() {
  ...
  if (error) return <div className="h-full flex flex-col items-center justify-center p-10 text-red-500"><p>{error}</p><Button className="mt-4" onClick={() => router.push('/dashboard/admin/approvals')}>Back to Approvals</Button></div>
  if (!property) return <div className="h-full flex items-center justify-center p-10">Property not found</div>

  return (
    <div className="space-y-6 pb-20 max-w-6xl mx-auto">
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" onClick={() => router.push('/dashboard/admin/approvals')}>
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Review Property</h1>
          <p className="text-muted-foreground">Examine details and documents before approving this listing.</p>
        </div>
      </div>
        ...
        <div className="flex-1 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-6">
              <div>
                <div className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-md text-sm font-bold uppercase tracking-wider inline-block mb-3">
                  {property.status}
                </div>
